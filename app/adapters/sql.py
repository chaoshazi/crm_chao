...
class SqlStore:
    ...
    def stats(self, actor: Actor) -> dict[str, Any]:
        ...
        with self.engine.begin() as conn:
            ...
            opp = self.table('opportunities')
            opp_conds: list[Any] = [
                opp.c.tenant_id == actor.tenant_id,
                opp.c.deleted_at.is_(None),
                *self._visibility_conditions(actor, OBJECTS['opportunities'], opp),
            ]
            rows = conn.execute(
                select(opp.c.stage, func.count(), func.coalesce(func.sum(opp.c.amount), 0))
                .where(*opp_conds)
                .group_by(opp.c.stage)
            ).all()
            pipeline = [
                {
                    'stage': str(row[0] or 'new'),
                    'count': int(row[1]),
            ...
            task_conds: list[Any] = [
                tasks.c.tenant_id == actor.tenant_id,
                tasks.c.deleted_at.is_(None),
                *self._visibility_conditions(actor, OBJECTS['tasks'], tasks),
                or_(tasks.c.status.is_(None), tasks.c.status != TASK_DONE),
            ]
            open_tasks = int(
                conn.execute(select(func.count()).select_from(tasks).where(*task_conds)).scalar_one()
            )
        recent = self.list_records(actor, 'activities', limit=5, order='desc', page=1).items
        return {
            'counts': counts,
            'pipeline': pipeline,
            'pipeline_amount': float(sum(item['amount'] for item in pipeline)),
            'won_amount': float(
                sum(item['amount'] for item in pipeline if item['stage'] == 'closed_won')
            ),
