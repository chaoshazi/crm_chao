...
同一个进程既提供 API 也托管 CRM 前端构建产物（web/dist）：
跑一次 cd web; npm run build 之后，直接访问 http://127.0.0.1:9100 就是完整界面；
...
def create_app(settings: Settings | None = None) -> FastAPI:
    ...
    application = FastAPI(
        title='CRM 管理系统',
        version=__version__,
        description='自研销售 CRM：业务管理界面 + 供 AI 能力层使用的同步与写回接口。',
        lifespan=lifespan,
    )

    @application.exception_handler(CrmError)
    async def _crm_error(request: Request, exc: CrmError) -> JSONResponse:
...
def _mount_web(application: FastAPI, settings: Settings) -> None:
    ...
    if not settings.serve_web:
        logger.info('CRM_SERVE_WEB=false，跳过前端静态托管')
        return
