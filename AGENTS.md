...
`D:\crm` 是独立的自研 CRM 管理系统（FastAPI + SQLAlchemy Core + Vite/React 前端），
...
python -m uvicorn app.main:app --host 127.0.0.1 --port 9100 # 起后端（默认 sqlite）

cd D:\crm\web
npm install
npx tsc --noEmit                                          # 前端类型检查
npm run dev                                               # 开发服务器 5174，代理到 9100
...
```powershell
cd D:\crm\web; npm run build
cd D:\crm; docker compose up -d --build
```
...
- `app/contract.py` 是对象与字段元数据的唯一来源。
  **改字段名等于改与 AI 能力层的对接契约**，要和 `D:\codex` 的
  `app/integrations/crm/field_map.py` 同步改。
