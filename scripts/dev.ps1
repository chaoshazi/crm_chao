# 起 CRM 本地开发环境：后端 9100（后台，日志写 logs/） + 前端 5174（前台，热更新）
#
# 用法： powershell -ExecutionPolicy Bypass -File D:\crm\scripts\dev.ps1
#        只起后端： 加 -ApiOnly
#
...
if ($ApiOnly) {
    Write-Host ('启动 CRM API: http://127.0.0.1:{0}' -f $apiPort)
...
Write-Host ('CRM API 已后台启动 pid={0}，日志: {1}' -f $api.Id, $apiOut)
Write-Host ('健康检查: http://127.0.0.1:{0}/health' -f $apiPort)
Write-Host ('CRM 界面: http://127.0.0.1:{0}（Ctrl+C 结束，后端会一起停）' -f $webPort)
