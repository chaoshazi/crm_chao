...
export default function App() {
  ...
  const loadRefs = useCallback(async () => {
    ...
    const [userRows, teamRows, accounts, leads, opportunities, contacts] = await Promise.all([
      safeList('users'),
      safeTeams(),
      safeList('accounts'),
      safeList('leads'),
      safeList('opportunities'),
      safeList('contacts'),
    ]);
    ...
    collect(accounts.data, 'name');
    collect(leads.data, 'company');
    collect(opportunities.data, 'name');
    collect(contacts.data, 'name');
    setUsers(userRows.data.map(actorOf));
    setTeams(teamRows.data);
    setCtx({ recordTitles, teamNames, userNames });
    setRefOptions({
      accounts: accounts.data.map((row) => ({ value: row.id, label: String(row.name ?? row.id) })),
  ...
  return (
    <div className='flex h-full'>
      <aside className='flex w-56 shrink-0 flex-col border-r border-slate-800 bg-slate-900/40 p-3'>
        <h1 className='mb-1 text-base font-semibold text-slate-100'>CRM 管理系统</h1>
        <p className='mb-4 text-xs text-slate-500'>销售业务管理后台</p>
