    ...
    <div className='flex flex-col gap-3'>
      ...
      <Card title='AI 能力层对接'>
        <p className='mb-3 text-xs text-slate-400'>
          在 D:\codex（AI 能力层）里把 CRM 从 fake 切到 rest 即可使用本系统作为数据源。
        ...
        <Code
          lines={[
            'AICRM_CRM_MODE=rest',
            'AICRM_CRM_BASE_URL=' + origin + '/api/v1',
            'AICRM_CRM_API_TOKEN=<与 CRM_SERVICE_TOKEN 相同的值>',
            'AICRM_CRM_AUTH_MODE=bearer',
          ]}
        />
        <p className='mb-1 mt-3 text-xs font-semibold text-slate-300'>接入前先跑只读探测（在 D:\\codex 目录）</p>
        <Code
          lines={[
            'python scripts/crm_probe.py --base-url ' +
              origin +
              '/api/v1 --token <CRM_SERVICE_TOKEN> --all --show-raw',
          ]}
        />
      </Card>
      ...
      <Card title='服务启动提示'>
        ...
        <Code lines={['cd D:\\crm\\web', 'npm install', 'npm run dev   # http://127.0.0.1:5174']} />
        <p className='mb-1 mt-3 text-xs font-semibold text-slate-300'>生产形态（Postgres + 托管前端）</p>
        <Code lines={['cd D:\\crm\\web', 'npm run build', 'cd D:\\crm', 'docker compose up -d']} />
      </Card>
    </div>
  );
}